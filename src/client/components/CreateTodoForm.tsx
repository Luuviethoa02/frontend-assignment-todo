...
export const CreateTodoForm = () => {
  ...
  const handleInputChanges: KeyboardEventHandler<HTMLInputElement> = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault()
      createTodo({
        body: todoBody,
  ...
  return (
    <form className="group flex items-center justify-between rounded-12 border border-gray-200 py-2 pr-4 focus-within:border-gray-400">
      ...
      <button
        ...
        className="left-5 rounded-[9.99999px] bg-[#334155] px-5 py-2 text-center text-sm font-bold text-white"
        onClick={() => {
          createTodo({
            body: todoBody,
          })
