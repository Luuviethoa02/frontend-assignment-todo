import type { TabValue } from '@/pages'
...
export const TodoList = ({ tab }: Iprops) => {
  ...
  return (
    <ul ref={parent} className="grid grid-cols-1 gap-y-3 pt-10">
      {todos.length > 0 &&
        todos
          .filter((todo) => (tab === 'all' ? true : todo.status === tab))
          .map((todo) => (
            <li key={todo.id}>
              <div
              ...
              >
                <div className="flex items-center">
                  ...
                  {todo.status === 'completed' && (
                    <label
                      className="block pl-3 font-medium line-through"
      ...
      {(todos.length === 0 ||
        todos.filter((todo) => (tab === 'all' ? true : todo.status === tab))
          .length === 0) && (
        <li>
